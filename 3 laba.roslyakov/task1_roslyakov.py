...
class AudioBook(Book):
    ...
    def __repr__(self):
        return (
            f"{self.__class__.__name__}(name={self.name!r}, author={self.author!r}, duration={self.duration:.2f})"
        )


# Примеры использования
if __name__ == "__main__":
    paper_book = PaperBook(name="Преступление и наказание", author="Ф. Достоевский", pages=671)
    print(paper_book)
    print(repr(paper_book))

    audio_book = AudioBook(name="1984", author="Дж. Оруэлл", duration=11.5)
    print(audio_book)
